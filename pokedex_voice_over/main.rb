...
module PokedexVoiceOver
  ...
  # Write the hidden-launch VBScript shim used on Windows so the Fish-Speech
  # server's console window never appears on screen or in the taskbar.
  # WshShell.Run(cmd, 0, False): 0 = hidden window, False = don't wait.
  # The shim reconstructs a quoted command line:  "<bat>" --parent-pid <pid>
  def self._write_run_hidden_vbs(path)
    return if FileTest.exist?(path)
    vbs = <<~'VBS'
      ' Auto-generated by Pokedex Voice Over. Launches a command fully hidden
      ' (no window, no taskbar button). Args: 0 = bat path, 1 = parent pid.
      Set sh = CreateObject("WScript.Shell")
      bat = WScript.Arguments(0)
      ppid = WScript.Arguments(1)
      sh.Run """" & bat & """ --parent-pid " & ppid, 0, False
    VBS
    begin
      File.open(path, "w") { |f| f.write(vbs) }
    rescue => e
      log("  fish-speech autostart: could not write run_hidden.vbs: #{e.class}: #{e.message}")
    end
  end
  ...
  def self._fish_speech_try_autostart
    ...
    begin
      ...
      if win
        ...
        if debug_spawn
          # Debug: visible window so first-run / setup output is readable.
          spawn_cmd = %Q{start "FishSpeechTTS" "#{launcher_win}" #{parent_pid_arg}}
          log("  fish-speech autostart: cmdline => #{spawn_cmd}")
          ok = system(spawn_cmd)
        else
          # Normal play: launch completely HIDDEN — no console window and no
          # taskbar button — via a tiny VBScript shim run by wscript.exe
          # (WshShell.Run with windowStyle 0 = hidden).  The server still
          # writes its full transcript to fish_speech/setup.log, and the
          # --parent-pid monitor + at_exit/crash hooks still shut it down.
          vbs_abs = File.expand_path("#{FISH_SPEECH_DIR}/run_hidden.vbs")
          _write_run_hidden_vbs(vbs_abs)
          vbs_win = vbs_abs.gsub('/', '\\')
          # wscript args: <bat path> <parent pid>.  Each quoted so spaces /
          # parentheses / apostrophes in the install path survive tokenising.
          spawn_cmd = %Q{wscript "#{vbs_win}" "#{launcher_win}" #{Process.pid}}
          log("  fish-speech autostart: hidden cmdline => #{spawn_cmd}")
          ok = system(spawn_cmd)
        end
      else
  ...
  def self.play(species, fused_species = nil, displayed_text = nil)
    ...
    @playback_active = true  # a voice-over is now committed/playing (see playback_active?)
  ...
  # True while a voice-over has been started by play() and not yet stopped.
  # Lets the scene hooks cancel a lingering entry exactly once when the
  # cursor lands on an unowned (???) entry, without thrashing BGM. This makes
  # owned->unowned behave the same on every base (fork or OG KIF) instead of
  # relying on the engine to cancel the previous playback.
  def self.playback_active?
    !!@playback_active
  end
...
if defined?(PokemonPokedexInfo_Scene)
  ...
  class PokemonPokedexInfo_Scene
    ...
    # True only when the Pokédex entry TEXT is actually visible to the player.
    # KIF draws the flavour text in drawPageInfo only `if $Trainer.owned?(@species)`
    # (seen-but-not-owned shows ??? with no entry), so we mirror that exact
    # condition: never narrate an entry the player can't read. The real-time
    # "just caught / fused / evolved" path goes through pbStartSceneBrief, which
    # is intentionally NOT gated (ownership is registered the moment you obtain
    # the species, and that path is only ever reached for a species you now own).
    def pokedex_vo_entry_visible?
      return true unless (defined?($Trainer) && $Trainer && $Trainer.respond_to?(:owned?))
      sp = instance_variable_defined?(:@species) ? @species : nil
      return true if sp.nil?   # can't tell -> don't suppress
      begin
        owned = $Trainer.owned?(sp)
        PokedexVoiceOver.log("  pokedex_vo_entry_visible?: owned?(#{sp.inspect}) => #{owned.inspect}")
        return owned ? true : false
      rescue StandardError => e
        PokedexVoiceOver.log("  pokedex_vo_entry_visible? error (#{e.class}: #{e.message}) — allowing")
        return true
      end
    end
    ...
    if method_defined?(:pbShowPage)
      ...
      def pbShowPage(page, *args)
        ...
        if page == POKEDEX_VO_ENTRY_PAGE && pokedex_vo_entry_visible?
          head, fused = pokedex_vo_current_species
          current_species = [head, fused]
        ...
        elsif page == POKEDEX_VO_ENTRY_PAGE
          # Entry page, but this species is unowned (??? — no readable entry).
          # Stop any voice still bleeding over from the previous owned entry.
          # Guarded by playback_active? so we stop exactly once on the
          # transition and never thrash BGM while parked on the unowned entry.
          if PokedexVoiceOver.playback_active?
            PokedexVoiceOver.log("  pbShowPage: moved to unowned/empty entry — stopping voice")
      ...
      if method_defined?(:drawPage)
        ...
        def drawPage(page, *args)
          ...
          if page == POKEDEX_VO_ENTRY_PAGE && !@dex_vo_in_brief && pokedex_vo_entry_visible?
            head, fused = pokedex_vo_current_species
            current_species = [head, fused]
            species_changed = current_species != @dex_vo_last_species
            page_changed = prev_page != POKEDEX_VO_ENTRY_PAGE
          ...
          elsif page == POKEDEX_VO_ENTRY_PAGE && !@dex_vo_in_brief
            # Entry page, but this species is unowned (??? — no readable entry).
            # Stop any voice still bleeding over from the previous owned entry.
            # Guarded by playback_active? so we stop exactly once on the
            # transition and never thrash BGM while parked on the unowned entry.
            if PokedexVoiceOver.playback_active?
              PokedexVoiceOver.log("  drawPage: moved to unowned/empty entry — stopping voice")
              PokedexVoiceOver.stop
            end
            @dex_vo_pending_play = false
          end
...
# =============================================================================
# In-game toast notification — signals when the Fish-Speech TTS server is ready
# =============================================================================
# A small, translucent pill appears in the bottom-right corner of the screen
# for ~4 seconds whenever the Fish-Speech server finishes loading and is ready
# to synthesise voice-over audio.  It fades in/out automatically and never
# blocks input.
#
# Architecture:
#   PokedexVoiceOver::Toast  — manages the sprite lifecycle (main-thread only)
#   Scene_Map#update hook    — ticks the toast every frame during overworld play
#   Background poller thread — polls /health and queues the toast on first success
# =============================================================================

module PokedexVoiceOver
  module Toast
    SHOW_SECS     = 4.0   # total on-screen time (seconds)
    FADE_IN_SECS  = 0.4   # time to reach full opacity
    FADE_OUT_SECS = 0.6   # time to fade to transparent
    TOAST_W       = 240
    TOAST_H       = 36

    class << self
      # Queue a message to display on the next main-thread update tick.
      # Safe to call from a background thread — Ruby's GIL makes the assignment
      # atomic; the main thread reads it exactly once in update.
      def queue(msg)
        @pending = msg
      end

      # Called every frame from Scene_Map#update (main thread only).
      # Creates the sprite on first call after queue, then animates opacity.
      def update
        # Start a new toast if one is queued and no sprite is active
        if @pending && (@sprite.nil? || (@sprite.respond_to?(:disposed?) && @sprite.disposed?))
          _create(@pending)
          @pending = nil
          @t0      = Time.now.to_f
        end

        return unless @sprite && !@sprite.disposed? && @t0

        elapsed = Time.now.to_f - @t0
        if elapsed < FADE_IN_SECS
          @sprite.opacity = ((elapsed / FADE_IN_SECS) * 255).to_i.clamp(0, 255)
        elsif elapsed < SHOW_SECS - FADE_OUT_SECS
          @sprite.opacity = 255
        elsif elapsed < SHOW_SECS
          frac = (elapsed - (SHOW_SECS - FADE_OUT_SECS)) / FADE_OUT_SECS
          @sprite.opacity = ((1.0 - frac) * 255).to_i.clamp(0, 255)
        else
          _dispose
        end
      rescue => e
        PokedexVoiceOver.log("Toast.update error: #{e.class}: #{e.message}") rescue nil
        _dispose rescue nil
      end
      ...
      def _create(msg)
        _dispose
        begin
          # High-Z viewport so the pill renders above all game UI layers
          @vp      = Viewport.new(0, 0, Graphics.width, Graphics.height)
          @vp.z    = 99998

          bm = Bitmap.new(TOAST_W, TOAST_H)

          # Dark semi-transparent background
          bm.fill_rect(0, 0, TOAST_W, TOAST_H, Color.new(15, 15, 15, 210))
          # Blue accent bar on left edge
          bm.fill_rect(0, 0, 4, TOAST_H, Color.new(80, 180, 255, 255))

          bm.font.size  = 17
          bm.font.bold  = true
          bm.font.color = Color.new(240, 240, 240, 255)
          bm.draw_text(10, 0, TOAST_W - 14, TOAST_H, msg)

          @sprite         = Sprite.new(@vp)
          @sprite.bitmap  = bm
          # Bottom-right corner, 10 px from edge
          @sprite.x       = Graphics.width  - TOAST_W - 10
          @sprite.y       = Graphics.height - TOAST_H - 10
          @sprite.z       = 99998
          @sprite.opacity = 0
        rescue => e
          PokedexVoiceOver.log("Toast._create error: #{e.class}: #{e.message}") rescue nil
          _dispose
        end
      end
...
# Hook Scene_Map#update so Toast.update is ticked every game frame
# while the player is on the overworld — where the server will most likely
# finish loading.
begin
  if defined?(Scene_Map) && Scene_Map.method_defined?(:update) &&
     !Scene_Map.method_defined?(:dex_vo_toast_orig_update)
    class Scene_Map
      alias dex_vo_toast_orig_update update
      def update
        dex_vo_toast_orig_update
        PokedexVoiceOver::Toast.update rescue nil
      end
    end
    PokedexVoiceOver.log("Scene_Map#update hooked for server-ready toast")
  end
rescue => e
  PokedexVoiceOver.log("Scene_Map toast hook error: #{e.class}: #{e.message}") rescue nil
end

# Background poller: once the Fish-Speech server passes a health check, queue
# the toast so the player is notified on the next overworld frame.
# Uses _prefetch_server_ready? (same as the idle prefetch worker) so it waits
# until the server is both up AND not mid background-compile.
if PokedexVoiceOver.fish_speech_enabled?
  Thread.new do
    begin
      sleep(4)   # let the auto-start thread spawn first
      deadline   = Time.now.to_f + 600   # give up after 10 minutes
      last_log_t = Time.now.to_f
      until Time.now.to_f > deadline
        if PokedexVoiceOver._prefetch_server_ready?
          PokedexVoiceOver.log("Server-ready toast: Fish-Speech confirmed up — queuing toast")
          PokedexVoiceOver::Toast.queue("Voice server ready!")
          break
        end
        now = Time.now.to_f
        if now - last_log_t > 30
          PokedexVoiceOver.log("Server-ready toast: still waiting for Fish-Speech server...")
          last_log_t = now
        end
        sleep(3)
      end
    rescue => e
      PokedexVoiceOver.log("Server-ready toast poller error: #{e.class}: #{e.message}") rescue nil
    end
  end
  PokedexVoiceOver.log("Server-ready toast: background poller started")
end
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Crash-close: KIF shows its error box via the top-level pbPrintException(e).
# at_exit/SIGTERM are unreliable on a crash (MKXP-Z may force-close, and the
# error box blocks until the player clicks OK), so we hook pbPrintException to
# shut the Fish-Speech sidecar down the instant a crash is reported — its
# hidden terminal goes away immediately instead of lingering until the game
# process finally dies.  Installed on Object because pbPrintException is a
# private top-level method under MKXP-Z's Ruby.
begin
  if (Object.private_method_defined?(:pbPrintException) ||
      Object.method_defined?(:pbPrintException)) &&
     !Object.private_method_defined?(:__pdvo_orig_pbPrintException)
    Object.class_eval do
      alias_method :__pdvo_orig_pbPrintException, :pbPrintException
      def pbPrintException(e)
        begin
          PokedexVoiceOver._fish_speech_shutdown
        rescue Exception
          # never let TTS cleanup interfere with showing the real crash
        end
        __pdvo_orig_pbPrintException(e)
      end
    end
    PokedexVoiceOver.log("pbPrintException crash hook installed") rescue nil
  end
rescue => _e
  # If anything goes wrong installing the hook, fall back to at_exit/monitor.
end
...
class PokedexVoiceOverSettingsScene < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    ...
    options << EnumOption.new(
      ...
      _INTL("Mute music while the Pok\xc3\xa9dex entry is being read (e.g. after capture)")
    )
  ...
  def pbStartScene(inloadscreen = false)
    ...
    @sprites["title"] = Window_UnformattedTextPokemon.newWithSize(
      _INTL("Pok\xc3\xa9dex Voice Over"), 0, 0, Graphics.width, 64, @viewport)
...
# =============================================================================
# Registers a single "Pok\xc3\xa9dex Voice Over" button under the Interface category
...
pokedex_vo_register_settings = proc {
  ...
  ModSettingsMenu.register(:pokedex_vo_submenu, {
    name: "Pok\xc3\xa9dex Voice Over",
    type: :button,
    description: "Configure Pok\xc3\xa9dex voice-over settings",
    on_press: proc {
      pbFadeOutIn {
