...
def kurayEncounterInit(encounter_type)
  ...
  # Only attempt fusion if the base encounter is NOT a UB/Paradox
  if isFusedEncounter() && !npt_global_species?(encounter[0])
    encounter_fusedWith = getEncounter(encounter_type)
    # Also skip if the fusion partner would be a UB/Paradox
    if encounter[0] != encounter_fusedWith[0] && !npt_global_species?(encounter_fusedWith[0])
